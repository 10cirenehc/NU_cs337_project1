...
class Strategy:
    def __init__(self, name: str, requirements: Optional[List[str]] = None):
        ...
        self.leftwords = ["win","wins","won","nominated","present","shouldn\'t have won","should have won","deserved","deserves","over"]
        self.rightwords = ["congrats","congratulations","robbing","goes to" "should\'ve been", "robbing", "should have been"]
        self.bothwords = ["winner","acceptance speech", "congratulate","should've", "robbed","rob","nominations","nominee","presented","presents"]
    ...
    def extract(self, data: List[Dict[str, Any]]) -> list[Dict[str, Any]]:
        ...
        for i in tqdm(range(len(data))):
            ...
            for award_regex in Award_Category.award_regex_dict.keys():
                if bool(re.search(award_regex, tweet['clean_text'],re.IGNORECASE)) or bool(re.search(award_regex, " ".join(tweet['hashtags']),re.IGNORECASE)):
                    found = True
                    names = []
                    titles = []
