...
def get_award_data(year):
    ...
    for award in tqdm(Award_Category.award_regex_dict.values()):
        ...
        award.nominees = list(sort_dict_by_value(award.potentialNominees).keys())[0:5]
        ans.append(award)
    ...
    for i in ans:
        tmp = dict()
        tmp['winner'] = i.winner
        tmp['presenters'] = i.presenters
        tmp['nominees'] = i.nominees
        result[i.name] = tmp
    json.dump(result, open(f"data/eric{year}_answers.json", "w"), indent=4)
    return result
        
if __name__ == "__main__":
    ans = get_award_data()

    # json.dump(result, open("data/my_answers.json", "w"), indent=4)
